...
//
// A portal constructs one of these once and passes it to the shared admin and
// account modules.

export interface TopicKeyConfig {
...
/** Clerk org metadata key holding the purchased seat count. */
export function teamSeatsKey(cfg: TopicKeyConfig): string {
  return `${cfg.topic}_team_seats`;
}

/**
 * Retained name. The admin modules were the first consumers, and renaming the
 * exported type would churn every portal's admin config for no behaviour gain.
 */
export type AdminConfig = TopicKeyConfig;
